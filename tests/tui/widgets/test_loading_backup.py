...
class TestLoadingIndicator:
    ...
    def test_loading_indicator_initialization_defaults(self) -> None:
        ...
        assert (
            indicator._pulse_effect is False
        )  # noqa: SLF001  # Testing internal state initialization
        assert (
            indicator._frame_index == 0
        )  # noqa: SLF001  # Testing internal state initialization
        assert (
            indicator._frames == []
        )  # noqa: SLF001  # Testing internal state initialization
        assert (
            indicator._animation_timer is None
        )  # noqa: SLF001  # Testing internal state initialization
    ...
    def test_loading_indicator_initialization_custom(self) -> None:
        ...
        assert indicator._pulse_effect is True
    ...
    def test_spinner_setup(self) -> None:
        ...
        with patch.object(indicator, "_update_spinner_display") as mock_update:
            indicator._setup_spinner()

            # Should set up frames and reset index
            assert indicator._frames == SpinnerFrames.get_frames("line")
    ...
    async def test_loading_indicator_start_stop_animation(self) -> None:
        ...
        indicator = LoadingIndicator()
        indicator._frames = ["⠋", "⠙", "⠹"]  # Set up test frames
        ...
        with patch.object(
            ...
            indicator._start_animation()

            mock_set_interval.assert_called_once()
            assert indicator._animation_timer is mock_timer

            # Test stopping animation
            indicator._stop_animation()
    ...
    def test_animate_spinner_frame_progression(self) -> None:
        ...
        indicator = LoadingIndicator()
        indicator._frames = ["A", "B", "C"]
        indicator._frame_index = 0

        with patch.object(indicator, "_update_spinner_display") as mock_update:
            # First animation step
            indicator._animate_spinner()
            assert indicator._frame_index == 1
            mock_update.assert_called()

            # Second animation step
            mock_update.reset_mock()
            indicator._animate_spinner()
            assert indicator._frame_index == 2
            mock_update.assert_called()
    ...
    def test_update_spinner_display(self) -> None:
        ...
        with patch.object(
            ...
            indicator._update_spinner_display()
    ...
    def test_theme_initialization_success(self) -> None:
        ...
        with patch("src.tui.widgets.loading.ThemeManager") as mock_theme_class:
            ...
            indicator._initialize_theme()

            assert indicator._theme_manager is mock_manager
    ...
    def test_theme_initialization_failure(
        ...
        with (
            ...
            assert indicator._theme_manager is None
...
class TestProgressIndicator:
    ...
    def test_progress_indicator_initialization_defaults(self) -> None:
        ...
        assert indicator._show_count is True
        assert isinstance(indicator._start_time, float)
    ...
    def test_progress_indicator_initialization_custom(self) -> None:
        ...
        assert indicator.current == 25
        assert indicator.total == 200
        assert indicator.show_eta is False
        assert indicator._show_count is False
    ...
    def test_progress_bar_update(self) -> None:
        ...
        with patch.object(
            ...
            indicator._update_progress_bar()

            assert mock_progress_bar.progress == 30
    ...
    def test_progress_bar_update_clamps_value(self) -> None:
        ...
        with patch.object(
            ...
            assert mock_progress_bar.progress == 50  # Clamped to total
    ...
    def test_eta_calculation_insufficient_data(self) -> None:
        ...
        indicator.current = 0
        eta_text = indicator._calculate_eta_text()
        assert eta_text == "ETA: --"
    ...
    def test_eta_calculation_recent_start(self) -> None:
        ...
        indicator = ProgressIndicator()
        indicator._start_time = time.time()  # Just started
        indicator.current = 5
    ...
    def test_eta_calculation_valid_progress(self) -> None:
        ...
        indicator = ProgressIndicator(total=100)
        indicator._start_time = time.time() - 10  # 10 seconds ago
    ...
    def test_eta_time_formatting(
        ...
        formatted = indicator._format_eta_time(seconds)
    ...
    def test_completion_state_management(self) -> None:
        ...
        indicator.current = 25
        indicator._update_completion_state()
    ...
    def test_progress_indicator_convenience_methods(self) -> None:
        ...
        with patch.object(indicator, "_update_progress") as mock_update:
            ...
            assert indicator.current == 30
            assert indicator.total == 150
            mock_update.assert_called()

            # Test set_progress without total change
            mock_update.reset_mock()
            indicator.set_progress(45)
            assert indicator.current == 45
            assert indicator.total == 150  # Unchanged
            mock_update.assert_called()
    ...
    def test_progress_reset_functionality(self) -> None:
        ...
        start_time_before = indicator._start_time
        ...
        assert indicator.current == 0
        assert indicator.total == 200
        assert indicator._start_time > start_time_before
        assert "completed" not in indicator.classes
    ...
    def test_progress_completion_method(self) -> None:
        ...
        assert indicator.current == 75  # Set to total
        assert "completed" in indicator.classes
    ...
    def test_watch_methods_update_display(self) -> None:
        ...
        def mock_query_one(selector: str, widget_type: type[Any]) -> Mock:
            if selector == "#title":
