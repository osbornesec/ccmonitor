...
class TrendAnalyzer:
    ...
    def analyze_temporal_patterns(
        ...
        return {
            ...
            "message_frequency_trend": self._calculate_frequency_trend(
                timestamps
            ),
            "first_message": (
                sorted_timestamps[0].isoformat() if timestamps else None
            ),
            "last_message": (
                sorted_timestamps[-1].isoformat() if timestamps else None
            ),
        }
    ...
    def analyze_conversation_flow(
        ...
        for message in messages:
            if prev_message:
                ...
                if (
                    prev_message.get("timestamp")
                    and message.get("timestamp")
                    and prev_message.get("type") == "user"
        ...
        return {
            "user_assistant_ratio": (
                len(user_messages) / max(len(assistant_messages), 1)
            ),
    ...
    def predict_compression_benefits(
        ...
        # Predict optimal analysis level
        ...
        return {
            ...
            "optimal_monitoring_schedule": self._suggest_optimal_schedule(
                messages, historical_data
            ),
        }

    def _parse_timestamp(
        self, timestamp: Union[str, int, float]
    ) -> Optional[datetime]:
    ...
    def _analyze_verbose_content(
        ...
        for message in messages:
            ...
            is_verbose = (
                len(content) > 2000
                or content.count("```") > 2
                or (message.get("type") == "system" and len(content) > 500)
            )
    ...
    def _estimate_important_content_ratio(
        ...
        for message in messages:
            ...
            is_important = (
                ...
                or (
                    msg_type == "assistant"
                    and ("```" in content or len(content) > 200)
                )
...
class ReportFormatter:
    ...
    def format_table(self, stats: Dict[str, Any]) -> str:
        ...
        output.append(
            f"Average Length:      {stats.get('average_message_length', 0):.1f} characters"
        )
        output.append(
        ...
        if "compression_potential" in stats:
            ...
            for level, data in stats["compression_potential"].items():
                ...
                output.append(
                    f"{level.capitalize():12} {ratio:5.1f}% reduction ({format_size(size_reduction)})"
                )
            output.append("")
        ...
        if "analysis_time" in stats:
            ...
            output.append(
                f"Messages/Second:     {stats.get('total_messages', 0) / max(stats['analysis_time'], 0.001):,.0f}"
            )
            output.append("")
    ...
    def format_csv(self, stats: Dict[str, Any]) -> str:
        ...
        output.append(
            f"average_message_length,{stats.get('average_message_length', 0):.2f},basic"
        )
        output.append(
    ...
    def format_html(self, stats: Dict[str, Any]) -> str:
        ...
        html.append(
            f"                <div class='stat'><span class='label'>Total Messages:</span> <span class='value'>{stats.get('total_messages', 0):,}</span></div>"
        )
        html.append(
            f"                <div class='stat'><span class='label'>Average Length:</span> <span class='value'>{stats.get('average_message_length', 0):.1f} chars</span></div>"
        )
        html.append(
            f"                <div class='stat'><span class='label'>Total Characters:</span> <span class='value'>{stats.get('total_characters', 0):,}</span></div>"
        )

        if "file_size_bytes" in stats:
            html.append(
                f"                <div class='stat'><span class='label'>File Size:</span> <span class='value'>{format_size(stats['file_size_bytes'])}</span></div>"
            )
        ...
        if "message_types" in stats:
            ...
            for msg_type, count in stats["message_types"].items():
                html.append(
                    f"                <div class='stat'><span class='label'>{msg_type.capitalize()}:</span> <span class='value'>{count:,}</span></div>"
                )
            html.append("            </div>")
            html.append("        </div>")
        ...
        if "compression_potential" in stats:
            ...
            html.append("                <table>")
            html.append(
                "                    <tr><th>Level</th><th>Reduction</th><th>Size Saved</th></tr>"
            )
            for level, data in stats["compression_potential"].items():
                ratio = data.get("ratio", 0) * 100
                size_reduction = data.get("size_reduction", 0)
                html.append(
                    f"                    <tr><td>{level.capitalize()}</td><td>{ratio:.1f}%</td><td>{format_size(size_reduction)}</td></tr>"
                )
            html.append("                </table>")
            html.append("            </div>")
            html.append("        </div>")
        ...
        if "analysis_time" in stats:
            ...
            html.append("            <div class='stats-grid'>")
            html.append(
                f"                <div class='stat'><span class='label'>Analysis Time:</span> <span class='value'>{stats['analysis_time']:.3f} seconds</span></div>"
            )
            messages_per_sec = stats.get("total_messages", 0) / max(
                stats["analysis_time"], 0.001
            )
            html.append(
                f"                <div class='stat'><span class='label'>Messages/Second:</span> <span class='value'>{messages_per_sec:,.0f}</span></div>"
            )
            html.append("            </div>")
            html.append("        </div>")
...
class StatisticsGenerator:
    ...
    def analyze_file(
        ...
        try:
            ...
            if include_importance:
                result["importance_distribution"] = (
                    self._analyze_importance_distribution(messages)
                )
                result["compression_potential"] = (
                    self._analyze_compression_potential(messages)
                )

            if include_patterns:
                result["patterns_detected"] = self._analyze_patterns(messages)
                result["content_categories"] = self._categorize_content(
                    messages
                )
            ...
            result["analysis_performance"] = {
                ...
                "messages_per_second": (
                    len(messages) / max(analysis_time, 0.001)
                ),
            ...
            if self.verbose:
                logger.info(
                    f"Analysis complete: {len(messages)} messages in {analysis_time:.3f}s"
                )
    ...
    def analyze_directory(
        ...
        if self.verbose:
            logger.info(
                f"Analyzing directory: {directory} (recursive: {recursive})"
            )
        ...
        aggregated_stats = self._calculate_aggregated_statistics(
            per_file_stats, all_messages, total_size
        )
        ...
        result = {
            ...
            "cross_file_patterns": (
                self._analyze_cross_file_patterns(per_file_stats)
                if include_patterns
                else None
            ),
        }
        ...
        if include_trends and all_messages:
            result["directory_trends"] = (
                self.trend_analyzer.analyze_temporal_patterns(all_messages)
            )

        if self.verbose:
            logger.info(
                f"Directory analysis complete: {len(valid_files)} files, {len(all_messages)} messages in {analysis_time:.3f}s"
            )

        return result

    def simulate_analysis_levels(
        self, file_path: Path, levels: List[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for level in levels:
                threshold = {"light": 20, "medium": 40, "aggressive": 60}[
                    level
                ]
                preserved_count = 0
                ...
                for message in messages:
                    score = (
                        self.importance_scorer.calculate_message_importance(
                            message
                        )
                    )
                    if score >= threshold:
                ...
                compression_ratio = (
                    removed_count / len(messages) if messages else 0
                )

                # Estimate size reduction
                original_size = file_path.stat().st_size
                estimated_final_size = int(
                    original_size * (1 - compression_ratio * 0.8)
                )
                ...
                results[level] = {
                    ...
                    "estimated_focus_reduction_percent": (
                        (estimated_reduction / original_size * 100)
                        if original_size > 0
                        else 0
                    ),
                }
            ...
            logger.error(f"Analysis simulation failed for {file_path}: {e}")
            return {level: {"error": str(e)} for level in levels}

    def export_report(
        self, stats: Dict[str, Any], output_path: Path, format_type: str
    ):
        """
        ...
        try:
            ...
            if self.verbose:
                logger.info(
                    f"Report exported to {output_path} in {format_type} format"
                )
    ...
    def display_report(
        self, stats: Dict[str, Any], format_type: str = "table"
    ):
        """
        ...
        try:
            if format_type == "table":
                ...
                content = self.formatter.format_table(
                    stats
                )  # Default to table
    ...
    def _calculate_basic_statistics(
        self, messages: List[Dict[str, Any]], file_path: Path
    ) -> Dict[str, Any]:
    ...
    def _categorize_content(
        ...
        for message in messages:
            ...
            if msg_type == "system":
                ...
                term in content
                for term in ["error", "exception", "traceback", "debug"]
            ):
                categories["error_debugging"] += 1
            elif any(
                term in content
                for term in ["```", "function", "class", "import", "def"]
            ):
                categories["programming_content"] += 1
            elif any(
                term in content
                for term in ["documentation", "readme", "guide", "tutorial"]
            ):
    ...
    def _calculate_aggregated_statistics(
        ...
        total_messages = sum(
            stats.get("total_messages", 0) for stats in per_file_stats
        )
        total_chars = sum(
            stats.get("total_characters", 0) for stats in per_file_stats
        )
    ...
    def _analyze_file_diversity(
        ...
        diversity_level = (
            "high" if cv > 0.5 else "medium" if cv > 0.2 else "low"
        )
    ...
    def _analyze_cross_file_patterns(
        ...
        files_with_errors = len(
            [
                s
                for s in per_file_stats
                if s.get("content_categories", {}).get("error_debugging", 0)
                > 0
            ]
        )
        ...
        return {
            ...
            "error_prevalence": (
                files_with_errors / len(per_file_stats)
                if per_file_stats
                else 0
            ),
        }
    ...
    def _create_empty_result(
        self, file_path: str, analysis_time: float
    ) -> Dict[str, Any]:
