...
class StandardEntryValidator(EntryValidator):
    ...
    def __init__(self, validation_rule: ValidationRule) -> None:
        """Initialize standard entry validator with validation rule."""
    ...
    def get_validation_errors(self, entry: dict[str, Any]) -> list[str]:
        ...
        # Check if entry is a dictionary (early return if not)
        basic_structure_error = self._validate_basic_structure(entry)
        if basic_structure_error:
            return [basic_structure_error]

        # Check required fields
        field_errors = self._validate_required_fields(entry)
        errors.extend(field_errors)

        # Validate message content
        content_errors = self._validate_message_content(entry)
        errors.extend(content_errors)

        return errors

    def _validate_basic_structure(self, entry: dict[str, Any]) -> str | None:
        """Validate that entry is a dictionary."""
        if not isinstance(entry, dict):
            return "Entry must be a dictionary"
        return None

    def _validate_required_fields(self, entry: dict[str, Any]) -> list[str]:
    ...
    def _validate_message_content(self, entry: dict[str, Any]) -> list[str]:
        """Validate message structure and UUID."""
        errors = []
...
class JSONLParser:
    ...
    def __init__(self, validator: EntryValidator) -> None:
        """Initialize JSONL parser with entry validator."""
    ...
    def parse_file(self, file_path: Path) -> AnalysisResult:
        ...
        try:
            ...
            logger.exception("Failed to parse JSONL file")
            return AnalysisResult(
                valid_entries=[],
                malformed_count=0,
                total_lines=0,
    ...
    def _parse_file_content(self, file_path: Path) -> AnalysisResult:
        ...
        with file_path.open(encoding="utf-8") as f:
            for line_num, line in enumerate(f, 1):
                ...
                valid_entries, malformed_count = self._process_file_line(
                    stripped_line, line_num, valid_entries, malformed_count,
                )

        return self._generate_analysis_result(
            valid_entries, malformed_count, total_lines,
        )

    def _process_file_line(
        self,
        line: str,
        line_num: int,
        valid_entries: list[dict[str, Any]],
        ...
        return valid_entries, malformed_count
    ...
    def _log_validation_errors(self, line_num: int, entry: dict[str, Any]) -> None:
        ...
        for error in errors:
            logger.warning("Validation error at line %d: %s", line_num, error)
...
class DependencyMapper:
    ...
    def map_dependencies(self, entries: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        # First pass: create initial mapping
        self._create_initial_mapping(entries, dependency_map)

        # Second pass: link parent-child relationships
        self._link_parent_child_relationships(dependency_map, orphans)

        # Third pass: calculate depths
        self._calculate_depths(dependency_map)

        return self._finalize_dependency_data(dependency_map, orphans)

    def _create_initial_mapping(
        self, entries: list[dict[str, Any]], dependency_map: dict[str, dict[str, Any]],
    ) -> None:
    ...
    def _link_parent_child_relationships(
        self, dependency_map: dict[str, dict[str, Any]], orphans: list[str],
    ) -> None:
    ...
    def _finalize_dependency_data(
        self, dependency_map: dict[str, dict[str, Any]], orphans: list[str],
    ) -> dict[str, Any]:
        """Create final dependency data structure."""
        return {
    ...
    def _calculate_depths(self, dependency_map: dict[str, Any]) -> None:
        ...
        roots = [uuid for uuid, info in dependency_map.items() if not info["parent"]]
    ...
    def _calculate_depth_recursive(
        self, uuid: str, depth: int, dependency_map: dict[str, Any],
    ) -> None:
...
class ToolSequenceAnalyzer:
    ...
    def __init__(self, dependency_mapper: DependencyMapper) -> None:
        """Initialize tool sequence analyzer with dependency mapper."""
    ...
    def analyze_tool_sequences(self, entries: list[dict[str, Any]]) -> list[list[str]]:
        ...
        # Find tool call starting points and build sequences
        tool_starting_points = self._find_tool_starting_points(dependency_map)
        return self._build_sequences_from_starting_points(
            tool_starting_points, dependency_map, visited_tools,
        )

    def _find_tool_starting_points(
        self, dependency_map: dict[str, Any],
    ) -> list[tuple[str, dict[str, Any]]]:
        """Find all tool calls that can serve as sequence starting points."""
        tool_starting_points = []
        ...
        for uuid, info in dependency_map.items():
            ...
            if entry.get("type") == "tool_call":
                ...
                if tool_name:
                    tool_starting_points.append((uuid, info))

        return tool_starting_points

    def _build_sequences_from_starting_points(
        self,
        tool_starting_points: list[tuple[str, dict[str, Any]]],
        dependency_map: dict[str, Any],
        visited_tools: set[str],
    ) -> list[list[str]]:
        """Build tool sequences from identified starting points."""
        sequences = []

        for uuid, info in tool_starting_points:
    ...
    def _find_tool_child(
        self, children: list[str], dependency_map: dict[str, Any],
    ) -> str | None:
        """Find tool call child in children list."""
        for child_uuid in children:
            tool_result = self._check_child_for_tool(child_uuid, dependency_map)
            if tool_result:
                return tool_result
        return None

    def _check_child_for_tool(
        self, child_uuid: str, dependency_map: dict[str, Any],
    ) -> str | None:
        """Check a single child for tool calls."""
        if child_uuid not in dependency_map:
            return None
        ...
        if child_entry.get("type") == "assistant":
            return self._find_tool_in_grandchildren(child_uuid, dependency_map)

        return None

    def _find_tool_in_grandchildren(
        self, parent_uuid: str, dependency_map: dict[str, Any],
    ) -> str | None:
        """Find tool calls in grandchildren of a parent message."""
        grandchildren = dependency_map[parent_uuid]["children"]

        for grandchild_uuid in grandchildren:
            if grandchild_uuid not in dependency_map:
                continue

            grandchild_entry = dependency_map[grandchild_uuid]["entry"]
            if grandchild_entry.get("type") == "tool_call":
                return str(grandchild_uuid)
...
class RefactoredJSONLAnalyzer:
    ...
    def __init__(self, validation_rule: ValidationRule | None = None) -> None:
    ...
    def parse_jsonl_file(self, file_path: Path) -> list[dict[str, Any]]:
        ...
        if not result.success:
            raise InvalidJSONLError(
                result.error_message or "Failed to parse JSONL file",
            )
    ...
    def analyze_file(self, file_path: Path) -> dict[str, Any]:
        ...
        if not parse_result.success:
            raise InvalidJSONLError(
                parse_result.error_message or "Failed to parse file",
            )
        ...
        dependency_data = self.dependency_mapper.map_dependencies(entries)
        branch_analysis = self.branch_analyzer.analyze_branches(
            dependency_data["dependency_map"],
        )
        tool_sequences = self.tool_sequence_analyzer.analyze_tool_sequences(entries)
        circular_refs = self.circular_reference_detector.detect_circular_references(
            entries,
        )
        ...
        return {
            "parsing": {
                ...
                "success_rate": (
                    len(parse_result.valid_entries) / parse_result.total_lines
                    if parse_result.total_lines > 0
                    else 0
                ),
            },
...
class RefactoredConversationFlowAnalyzer:
    ...
    def __init__(self) -> None:
        """Initialize refactored conversation flow analyzer."""
        self.dependency_mapper = DependencyMapper()
        self.branch_analyzer = BranchAnalyzer()
        self.tool_sequence_analyzer = ToolSequenceAnalyzer(self.dependency_mapper)
        self.circular_reference_detector = CircularReferenceDetector()
    ...
    def map_conversation_dependencies(
        ...
        dependency_map: dict[str, Any] = dependency_data["dependency_map"].copy()
